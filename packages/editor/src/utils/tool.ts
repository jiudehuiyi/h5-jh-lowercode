 ...
 * @param {number} radix 生成uuid的进制，若为2，8，10则只会生成数字，若为16或者不传则会可能包含字母
 * @returns {string}
 */
...
export const getComponentsIndex = (
  cls = '#js_canvas .react-grid-item',
  dataProperty = 'data-componentId',
) => {
  const list = document.querySelectorAll(cls);
  const indexList = [...list].map((item) => {
    return item.getAttribute(dataProperty);
  });
  return indexList;
};
// 获取按顺序的pointData
export const getPointData = (_pointData: any) => {
  return getComponentsIndex().map((item) => {
    return _pointData.filter((point: any) => {
      if (item === point.id) {
        return point;
      }
    })?.[0];
