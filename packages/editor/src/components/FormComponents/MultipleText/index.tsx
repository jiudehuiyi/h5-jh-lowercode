/*
 * @Author: jiudehuiyi
 * @Date: 2022-09-28 11:22:48
 * @LastEditors:
 * @LastEditTime: 2022-09-28 11:23:20
 * @Description:
 */
...
import type { TMutiTextDefaultType } from '../types';
...
export default memo(function MutiText(props: MultiTextProps) {
  ...
  const handleDel = (index: number) => {
    const newList = value!.filter((_item, i) => i !== index);
    onChange && onChange(newList);
  };

  const handleChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const newList = value!.map((item, i) => (i === index ? e.target.value : item));
    onChange && onChange(newList);
  };
  useEffect(() => {
